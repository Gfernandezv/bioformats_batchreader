// -------------------------------------------------------------------
// Originally Written by: Patrice Mascalchi
// Date: 2019
...
// translated and modified by: Germán Fernández (2022), 
// Laboratorio de Neuroepigenética, Universidad Nacional Andres Bello
// g.fernndezvillalobos@uandresbello.edu
// -------------------------------------------------------------------
...
// V2:	- Split Channel for multichannel images.
// V2.1 - save images optimization.
...
Dialog.addString("File extension filter: (leave blank to disable)", ".lif");
Dialog.addMessage("");
Dialog.addChoice("For multi-channel images, save merge as RGB? ", yesno);
Dialog.addChoice("For multi-channel images, split channels? ", yesno, "yes");
...
print(outdir);
	...
	for (f=1;f<count+1;f++) {
		...
		print("*"+ snamePart +"* // *"+ tmpName +"*,   tamañoX="+ sizeX +" // "+tmpSizeX);
		...
		if (proceed==1) {
			print("Procesando: "+sname);
			run("Bio-Formats Importer", "open=[" + Mypath + Mylist[i] +"] autoscale color_mode=Default view=Hyperstack stack_order=XYCZT series_" + f);
			getDimensions(wi, he, ch, sl, fr);
			if (maxproj=="yes" && sl>1) run("Z Project...", "projection=[Max Intensity]");
			if (merge=="yes" && ch>1) run("Stack to RGB");
			//
			if (splitchannels=="yes" && ch>1) run("Split Channels");
				ch_nbr = nImages ; 
					for ( c = 1 ; c <= ch_nbr ; c++){
					selectImage(c);
					//outdir2 = Mypath + "channel" + c + File.separator;
					outdir2 = outdir + sname + File.separator;
						if (!File.exists(outdir2)) {
							File.makeDirectory(outdir2);
							}
							//if (!File.exists(outdir2)) exit("Cannot create export folder\n" + outdir2);
					currentImage_name = getTitle();
			saveAs("tiff", outdir2 + currentImage_name);
			print("guardado como: " + outdir2 + currentImage_name);
					}
//
			// Rename images 
			t = getTitle();
			...
			if (splitchannels=="yes" && ch>1) t =t+"_"+ch;
			// Only for multipositions filtering
			if (indexOf(t, "Pos0") > -1) t = replace(t, "Mark_and_Find_", "");
			
			if (ftyp=="tif" && splitchannels=="no") saveAs("Tiff", outdir + t + ".tif");
